...
use bincode::{deserialize, serialize, ErrorKind, Infinite};
...
impl DDS {
  ...
  pub fn parse_header_raw<R: io::Read>(buf: &mut R) -> Result<RawHeader, ParseError> {
    ...
    buf
      ...
      .expect("Not enough bytes to read magic bytes!");

    // If the file doesn't start with `DDS `, abort decoding
    if &magic_buf != b"DDS " {
      return Err(
        format!(
          "Expected the file to start with `DDS `, got `{}` instead.",
        ...
        data_buf
          ...
          .map(|p| {
            ...
            let convert = |mask: u32| -> u8 {
              ((pixel & mask) >> mask.trailing_zeros() * 255 / (2u32.pow(mask.count_ones()) - 1))
                as u8
            };
    ...
    layer_sizes
      ...
      .map(|&(height, width)| {
        ...
        let chunk_transform: Box<Fn(&[u8]) -> Vec<RGBA<u8>>> = match header.compression {
          Compression::DXT1 => Box::new(|chunk| DDS::bytes_to_pixels_dxt1(chunk, true)),
          Compression::DXT2 | Compression::DXT3 => {
            Box::new(|chunk| DDS::bytes_to_pixels_dxt3(chunk))
          }
          Compression::DXT4 | Compression::DXT5 => {
            Box::new(|chunk| DDS::bytes_to_pixels_dxt5(chunk))
          }
          _ => unreachable!(format!(
            "This function cannot handle `{:?}` images",
            header.compression
          )),
        };
  ...
  pub fn decode<R: io::Read>(buf: &mut R) -> Result<DDS, ParseError> {
    ...
    let layers = match header.compression {
      Compression::None => DDS::decode_uncompressed(&header, &mut data_buf),
      Compression::DXT1
      | Compression::DXT2
      | Compression::DXT3
      | Compression::DXT4
      | Compression::DXT5 => DDS::decode_dxt(&header, &mut data_buf),
      _ => {
        let compression_type = String::from_utf8_lossy(&header.fourcc[..]);
        return Err(
          format!(
            "The compression type `{}` is unsupported!",
            compression_type
          )
          .into(),
        );
  ...
  pub fn encode(
    ...
    if size.0 * size.1 < pixels.len() as u32 {
      return Err(
        format!(
          "Found {} extra bytes!",
          pixels.len() as u32 - size.0 * size.1
        )
        .into(),
      );
    }
    ...
    if size.0 * size.1 > pixels.len() as u32 {
      return Err(
        format!(
          "Need {} extra bytes!",
          size.0 * size.1 - pixels.len() as u32
        )
        .into(),
      );
    }
